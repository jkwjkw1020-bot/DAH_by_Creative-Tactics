...
NAVY, RED, GREEN = "#1c4a8a", "#c00000", "#1a7a4a"


def box(ax, x, y, w, h, text, fc="#cfe2f8", ec=NAVY, fs=9, tc="#111"):
    ax.add_patch(FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.02", fc=fc, ec=ec, lw=1.6))
    ax.text(x + w / 2, y + h / 2, text, ha="center", va="center", fontsize=fs, color=tc)


def arrow(ax, x1, y1, x2, y2, color="#333", lw=1.6):
...
def d_coop(p):
    ...
    ax.add_patch(Rectangle((0.2, 0.4), 3.5, 5.0, fc="#eaf2fb", ec="none"))
    ax.add_patch(Rectangle((6.3, 0.4), 3.5, 5.0, fc="#eafaf0", ec="none"))
    ax.text(1.95, 5.6, "MAVLink 도메인 (UAV)", ha="center", fontsize=9, color=NAVY)
    ax.text(8.05, 5.6, "ROS2/DDS 도메인 (UGV)", ha="center", fontsize=9, color=GREEN)
    box(ax, 0.5, 3.8, 2.9, 1.0, "정찰 UAV\n(ArduPilot · MAVLink)", "#cfe2f8")
    box(ax, 0.5, 1.0, 2.9, 1.0, "GCS\n(QGC · MAVProxy)", "#cfe2f8")
    box(ax, 4.0, 2.45, 2.0, 1.3, "협동\n게이트웨이\nMAVLink↔ROS2", "#ffd9d9", RED, 9)
    box(ax, 6.6, 3.8, 2.9, 1.0, "타격 UGV\n(ROS2 스택)", "#cdf0db", GREEN)
...
def d_killchain(p):
    ...
    for i, (t, tag) in enumerate(steps):
        fc = "#ffd9d9" if i == 1 else "#cfe2f8"; ec = RED if i == 1 else NAVY
        box(ax, x, 0.95, w, 1.0, t, fc, ec, 8.5)
        ax.text(x + w / 2, 0.7, tag, ha="center", fontsize=6.5, color="#666")
        if i < 5:
            arrow(ax, x + w, 1.45, x + w + gap, 1.45)
        x += w + gap
    ax.text(5.5, 2.45, "작전 MIRROR-CRACK — 게이트웨이 한 점에서 시작해 협동 체계 전체로 확산",
            ha="center", fontsize=9, color="#16335c")
    fig.savefig(p, dpi=150, bbox_inches="tight"); plt.close()
...
def d_defense(p):
    ...
    for name, desc, atk in layers:
        ...
        y += 1.2
    ax.text(4.2, 5.7, "JANUS-Blue 심층 방어 스택 (Zero-Trust)", ha="center", fontsize=10, color="#16335c")
    fig.savefig(p, dpi=150, bbox_inches="tight"); plt.close()


def d_janus(p):
    fig, ax = plt.subplots(figsize=(10, 6.2)); ax.set_xlim(0, 10); ax.set_ylim(0, 7.2); ax.axis("off")
    box(ax, 3.3, 6.1, 3.4, 0.85, "Orchestrator Agent (LLM)\n공방 진행 · 전략 · 설명 로그", "#fff2cc", "#bf9000", 9)
    ax.add_patch(Rectangle((0.3, 2.7), 3.9, 2.9, fc="#fff4f4", ec=RED, lw=1.2))
    ax.add_patch(Rectangle((5.8, 2.7), 3.9, 2.9, fc="#f1fbf5", ec=GREEN, lw=1.2))
    ...
    for i, t in enumerate(["Recon Agent", "Exploit Agent", "C2 / Impact Agent"]):
        box(ax, 0.6, 4.5 - i * 0.75, 3.3, 0.58, t, "#ffd9d9", RED, 8.5)
    ...
    for i, t in enumerate(["Sensor / IDS Agent", "Triage Agent", "Response / Recovery"]):
        box(ax, 6.1, 4.5 - i * 0.75, 3.3, 0.58, t, "#cdf0db", GREEN, 8.5)
    box(ax, 2.8, 0.5, 4.4, 1.0, "교전 환경 (ArduPilot SITL-lite + ROS2)\n공유 Blackboard · 텔레메트리", "#e6e6e6", "#555", 9)
    ...
    ax.text(5.15, 2.05, "공방 루프", fontsize=8, color="#888", ha="left")
    fig.savefig(p, dpi=150, bbox_inches="tight"); plt.close()
